...
namespace Ck {
	dbus_bool_t Session::ck_connector_open_graphic_session(const std::string &display, uid_t uid)
	{
		...
		return ck_connector_open_session_with_parameters(ckc, &error, "unix-user", &uid,
	...
	const char * Session::get_x11_device(const std::string &display)
	{
		...
		vt = *reinterpret_cast<long*>(return_value);
